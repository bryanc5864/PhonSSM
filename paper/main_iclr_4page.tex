...
\author{Jasper Zhang$^{1,*}$, Bryan Cheng$^{1,*}$, Austin Jin$^{1}$ \\
$^{1}$Great Neck South High School \\
\texttt{\{jasperzhang1001@gmail.com, bcbc7264@gmail.com, ahanchijin@gmail.com\}} \\
$^{*}$Equal contribution
}
...
\textbf{Few-Shot Performance.} Phonological decomposition enables strong few-shot learning: for signs with 1--5 training samples, 13.3\% vs 4.1\% (+225\% relative gain). The model also transfers zero-shot to held-out ASL Citizen samples: 64.1\% on overlapping vocabulary (vs 63.2\% supervised RGB baseline from \citet{desai2024asl}).
