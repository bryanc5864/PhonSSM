%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% PhonSSM: ICLR 2026 Paper (4-page version)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\author{Bryan Cheng$^{1}$, Austin Jin$^{1}$, Jasper Zhang$^{1}$ \\
$^{1}$William A. Shine Great Neck South High School \\
\texttt{\{bcbc7264@gmail.com, ahanchijin@gmail.com, jasperzhang1001@gmail.com\}} \\
}
...
\fancyhead{}
\fancyfoot{}
\fancyhead[L]{Workshop @ ICLR 2026}
\fancyfoot[C]{\thepage}
...
Consider the contrast between two strategies. \textit{Flat representations} assign each category an independent embedding; capacity scales as $O(K)$ with vocabulary size $K$. \textit{Compositional representations} factor categories into combinations of shared primitives; if $K$ categories arise from $M \ll K$ primitives, capacity scales as $O(M)$ while covering $O(M^c)$ combinations. This exponential gap explains why humans effortlessly generalize to novel words/signs sharing familiar components, while neural networks struggle.

\textbf{Sign language as a compositional testbed.} Sign languages exemplify this structure. Just as spoken words decompose into phonemes, signs decompose into \textit{cheremes}---minimal contrastive units including handshape ($\sim$30 categories), location ($\sim$15), movement ($\sim$10), and orientation ($\sim$8) \citep{stokoe1960sign,battison1978lexical}. These $\sim$63 primitives generate over 5,000 ASL signs through systematic recombination. The sign for ``mother'' differs from ``father'' only in location; ``chair'' differs from ``sit'' primarily in movement.

\textbf{Why current approaches fail.} Standard architectures (LSTMs \citep{hochreiter1997lstm}, Transformers \citep{vaswani2017attention}, GCNs \citep{kipf2017semi}) learn implicit flat representations. A Transformer may distinguish ``mother'' from ``father,'' but nothing ensures it has learned the location contrast that generalizes to other minimal pairs. The signature: poor few-shot performance and non-compositional errors.
...
\textbf{Contributions.} (1)~We formalize the \textit{compositional bottleneck}: flat representations have $O(K)$ capacity while compositional domains have $O(M^c)$ structure. (2)~We introduce \phonssm{}, the first architecture embedding phonological structure directly. (3)~State-of-the-art: 88.4\% WLASL100, \textbf{72.1\% WLASL2000} (+18.4pp over SOTA), 53.3\% on 5,565 signs. (4)~Causal evidence for compositionality: intervening on component embeddings flips predictions to minimal pairs 73.2\% of the time.
...
\phonssm{} processes landmarks $\mathbf{X} \in \mathbb{R}^{T \times N \times 3}$ ($T{=}30$ frames, $N{=}21$ or 75 landmarks) through four stages. Full details are in Appendix~\ref{app:theory}.
...
\textbf{Datasets.} We evaluate on two tracks: (1)~\textit{WLASL} \citep{li2020word}: standard benchmarks with 100/300/1000/2000 sign splits, using pose+hands (75 landmarks). We train separate models for each split. (2)~\textit{Merged-5565}: our new large-scale dataset combining 259,715 samples across 5,565 signs from ASL Citizen \citep{desai2023asl}, WLASL, and fingerspelling datasets; uses dominant hand only (21 landmarks) for consistency across sources.

\textbf{Implementation.} \phonssm{} uses model dimension $D{=}128$, component dimension $D_c{=}32$, 4 GAT attention heads, 4 BiSSM layers with expansion factor 2 (3.2M total parameters). Training: AdamW with lr $3{\times}10^{-4}$, cosine decay, batch size 128, 100 epochs. All results are mean over 3 seeds.

\begin{table}[t]
\caption{\textbf{Main results.} \textbf{Bold}: best skeleton method. $^\dagger$From \citet{hu2024dsta}.}
\label{tab:datasets}
\vspace{-2mm}
\begin{center}
\begin{small}
...
\toprule
Dataset & DSTA-SLR$^\dagger$ & I3D & \phonssm{} \\
\midrule
WLASL100 & 83.56 & 65.89 & \textbf{88.37} \\
WLASL300 & \textbf{80.00} & 56.14 & 74.41 \\
WLASL1000 & \textbf{67.81} & 47.33 & 62.90 \\
WLASL2000 & 53.70 & 32.48 & \textbf{72.08} \\
\midrule
Merged-5565 & -- & -- & \textbf{53.34} \\
\bottomrule
\end{tabular*}
\end{small}
\end{center}
...
\textbf{Main Results} (Table~\ref{tab:datasets}). On WLASL100, \phonssm{} reaches 88.37\% (+4.8pp over DSTA-SLR). On WLASL2000, we achieve \textbf{72.1\% vs 53.7\%} (+18.4pp). DSTA-SLR outperforms on mid-range splits (WLASL300/1000) due to \textit{minimal pair density}: mid-range vocabularies contain disproportionately more phonologically similar signs, favoring DSTA-SLR's fine-grained attention. At large vocabularies, compositional generalization dominates. On Merged-5565: 53.3\% vs 27.4\% Bi-LSTM (+26pp, $p{<}0.001$).

\textbf{Few-Shot Performance.} Phonological decomposition enables strong few-shot learning: for signs with 1--5 training samples, 13.3\% vs 4.1\% (+225\% relative gain). The model also transfers zero-shot to held-out ASL Citizen samples: 64.1\% on overlapping vocabulary (vs 63.2\% supervised RGB baseline from \citet{desai2023asl}).

\textbf{Analysis.} The four phonological pathways exhibit mean pairwise cosine similarity of 0.12 (vs 0.67 without $\mathcal{L}_{\text{ortho}}$), confirming orthogonality. Causal evidence for compositionality: swapping component embeddings for minimal pairs flips predictions to the partner sign 73.2\% of the time vs 12.4\% for controls ($p{<}0.001$). Additionally, 72\% of errors involve signs sharing 2+ phonological components (vs 8\% expected by chance).

\textbf{Ablations} (on WLASL100). Removing PDM: $-$11.9pp; replacing AGAN with MLP: $-$8.5pp; replacing BiSSM with LSTM: $-$6.2pp. \phonssm{} (3.2M params) runs at 260 samples/sec---12$\times$ faster than I3D (12.3M, video).
...
\textbf{Compositional generalization} has been studied in language \citep{lake2018generalization} and visual reasoning \citep{bahdanau2019systematic}. Prior work shows standard architectures fail at compositional extrapolation. We demonstrate that compositional inductive bias enables systematic generalization in recognition with $>$5,000 categories.

\textbf{Sign language recognition} evolved from hand-crafted features \citep{cooper2011sign} to video-based CNNs \citep{carreira2017quo} and skeleton-based GCNs \citep{yan2018spatial,li2020word,hu2024dsta}. DSTA-SLR achieves 53.7\% on WLASL2000 via fine-grained attention, but none exploit compositional linguistic structure.

\textbf{Phonological approaches.} Prior work used phonological features as auxiliary supervision \citep{koller2015continuous}. We make phonology architecturally central via explicit factorization and orthogonality constraints. Our approach relates to disentangled representations \citep{higgins2017beta} but grounds factorization in linguistic theory, adapting state space models \citep{gu2023mamba} and prototypical learning \citep{snell2017prototypical}.
...
\phonssm{} demonstrates that vocabulary scaling failures arise from representational mismatch: flat representations have $O(K)$ capacity while compositional domains have $O(M^c)$ structure. By embedding phonological structure directly, we achieve SOTA on WLASL100 (88.4\%) and WLASL2000 (72.1\%, +18.4pp), with +225\% few-shot improvement. Key findings: (1) compositionality requires explicit architectural constraints; (2) structure beats bandwidth (3.2M skeleton model outperforms 12.3M video model); (3) a precision-generalization tradeoff exists between compositional and discriminative approaches.
